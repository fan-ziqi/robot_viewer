...
export class TaktTimePanel {
    ...
    update({ store, nodeTimer }) {
        if (store && store !== this.store) { this._unsubStore?.(); this.store = store; this._unsubStore = store.onChanged?.(() => this._renderSoon()); }
    ...
    _subscribe() {
        this._unsubStore = this.store?.onChanged?.(() => this._renderSoon());
